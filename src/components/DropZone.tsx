// DropZone.tsx (ONNX version using local API proxy)
"use client";
...
export default function DropZone() {
  ...
  const processImage = useCallback(
    async (img: HTMLImageElement) => {
      ...
      for (let i = 0; i < img.width * img.height; i++) {
        ...
        floatData[i + img.width * img.height] = imageData.data[i * 4 + 1] / 255; // G
        floatData[i + img.width * img.height * 2] = imageData.data[i * 4 + 2] / 255; // B
      }

      const inputTensor = new ort.Tensor('float32', floatData, [1, 3, img.height, img.width]);
  ...
  return (
    <div className="p-4 max-w-lg mx-auto">
      <div {...getRootProps()} className="border-2 border-dashed rounded-xl p-6 text-center cursor-pointer">
        <input {...getInputProps()} />
        <p>{isDragActive ? 'Drop the image here...' : 'Drag and drop an image, or click to upload.'}</p>
      </div>

      {resultUrl && (
        <div className="mt-6">
          <h3 className="mb-2 font-semibold">Result</h3>
          <img src={resultUrl} alt="Processed" className="rounded-lg shadow-md" />
        </div>
      )}
    </div>
  );
